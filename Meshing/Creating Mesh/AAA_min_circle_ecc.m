...
for i=1:length(list)
    if isempty(strfind(list(i,:),'_T'))==0
        if isempty(strfind(list(i,:),'P13'))==0
            id_s(k)=i;
            k=k+1;
        end
    end
    if isempty(strfind(list(i,:),'ct_P13'))==0
        id_cl(m)=i;
        m=m+1
    end
end
...
for idcount=1:1%length(id_cl)
    ...
    filename = ['C:\Users\akshayrao\Documents\Rao Summer 17\AAA\Hamid_to_Akshay\P01to14\',a];
    delimiter = ' ';
    ...
    %% Centerline
    filename = ['C:\Users\akshayrao\Documents\Rao Summer 17\AAA\Hamid_to_Akshay\P01to14\',b];
    delimiter = ' ';
    ...
    for m=4:4:4%(length(B)-mod(length(B),4)) %index for moving through points of Center line
