...
for idcount=1:length(id_cl)
    ...
    X=[0 1 0];
    pint_ind=1;
    for m=4:4:(length(B)-mod(length(B),4))%%index of moving across centerline
        
        k=1; %the index for pts lying on this cross section
        tic
            ...
            for i=1:length(T)
                ...
                if(k1*k2<0 | k2*k3<0 | k1*k3<0)
                    ...
                    if u>=0 & v>=0 &u+v<=1
                        ...
                        k=k+1;
                        pt_counter=pt_counter+1;%the number of times the line intersects the mesh
