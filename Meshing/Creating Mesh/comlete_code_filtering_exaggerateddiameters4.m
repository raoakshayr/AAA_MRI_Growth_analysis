...
%             
            for j=1:w
                if isempty(Pint{pint_ind,j})==0
                    ptdist(j)=norm(B(m,1:3)-Pint{pint_ind,j});
                end
            end
            
            [maxdiafilt,maxdiafilt_ind]=max(ptdist);
            if mod(maxdiafilt_ind,2)==0
                max2_ind=maxdiafilt_ind-1;
            else
                max2_ind=maxdiafilt_ind+1;
            end
            
            ptdist_max(pint_ind)=ptdist(maxdiafilt_ind)+ptdist(max2_ind);%max dia filter
            if counter>1
                change=abs((ptdist_max(pint_ind)-ptdist_max(pint_ind-1))/ptdist_max(pint_ind-1));  
            else
                change=.05;
            end
            ptdist_max(pint_ind-1)
        end
    ...
    for i=1:l
        ...
        for j=1:w
            if isempty(Pintx{i,j})==0
                para{i,1}(j,1)=norm(B(i*4,1:3)-Pintx{i,j});
            end
        end
    end
