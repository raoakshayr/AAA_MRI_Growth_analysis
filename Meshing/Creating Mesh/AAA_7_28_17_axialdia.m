    ...
    for i=1:length(list)
        if patient<10
            if isempty(strfind(list(i,:),'_T'))==0
                if isempty(strfind(list(i,:),['P0',num2str(patient)]))==0
                    id_s(k)=i;
                    k=k+1
                end
            end
            if isempty(strfind(list(i,:),['ct_P0',num2str(patient)]))==0
                id_cl(m)=i;
                m=m+1
            end
        else
            ...
            if isempty(strfind(list(i,:),'_T'))==0
                if isempty(strfind(list(i,:),['P',num2str(patient)]))==0
                    id_s(k)=i;
                    k=k+1
                end
            end
            if isempty(strfind(list(i,:),['ct_P',num2str(patient)]))==0
                id_cl(m)=i;
                m=m+1
            end
        end
    end
    for idcount=5:length(id_s) 
        %% Patient ID
        a = strtrim(list(id_s(idcount),:)); %surface plot file
        pat_id=a;
        b=strtrim(list(id_cl(idcount),:)); %centerline plot
        %% Initialize variables.
        filename = ['C:\Users\akshayrao\Documents\Rao Summer 17\AAA\Hamid_to_Akshay\P19to26_axialdia\',a];
        delimiter = ' ';
        ...
        %% Centerline
        filename = ['C:\Users\akshayrao\Documents\Rao Summer 17\AAA\Hamid_to_Akshay\P19to26_axialdia\',b];
        delimiter = ' ';
        ...
        for m=4:4:(length(B)-mod(length(B),4)) %index for moving through points of Center line
            ...
            tic
            P1=cross(axialvector,X);%vector lying on cross section
            positive_pt=0;
            negative_pt=0;
            
            for theta=2*pi/20:2*pi/20:2*pi%increment lines in CS
                Pn=P1*cos(theta) + cross(axialvector,P1)*sin(theta)+ axialvector*dot(axialvector,P1)*(1-cos(theta));%%find increment vector
                P0=B(m,1:3);
                Dcs=axialvector(1)*B(m,1)+axialvector(2)*B(m,2)+axialvector(3)*B(m,3);
                for i=1:length(T)%run through all triangles
                    ...
                    positive_dist=10000;
                    negative_dist=10000;
                    k1=axialvector(1)*D(1)+axialvector(2)*D(2)+axialvector(3)*D(3)-Dcs;
                    k2=axialvector(1)*E(1)+axialvector(2)*E(2)+axialvector(3)*E(3)-Dcs;
                    k3=axialvector(1)*C(1)+axialvector(2)*C(2)+axialvector(3)*C(3)-Dcs;
            ...
            change%%%%%%%%%%%%%%%%%%%%filtering constant
            if change<.8 %only include CS if the number of pts is 40 or less or if there's a very large change
                pint_ind=pint_ind+1;
                cl_ind(cl_count)=m;
                cl_count=cl_count+1;
            end
        ...
        for k=1:l-flag
            for i=1:w
                ...
                if isempty(Pint{k,i})==0 & Pint{k,i}~=0
        ...
        for i=1:l-flag
            ...
            points=0;
            for j=1:w
                if isempty(Pintx{i,j})==0 & Pintx{i,j}~=0
        ...
        clearvars -except patient globmaxdia globmindia globmax_ecc globmax_per per_maxdia ecc_maxdia tort_disp_maxdia maxtort_disp tort_cl d idcount id_s id_cl list Pint para pat_id;
        save([pat_id(1:4),'.mat']);
        idcount=idcount+1;
        clearvars -except list pat_id patient id_s id_cl id
    end
end
