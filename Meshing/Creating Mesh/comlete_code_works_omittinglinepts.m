...
for i=1:length(list)
    if isempty(strfind(list(i,:),'_T'))==0
        if isempty(strfind(list(i,:),'P20'))==0
            id_s(k)=i;
            k=k+1;
        end
    end
    if isempty(strfind(list(i,:),'ct_P20'))==0
        id_cl(m)=i;
        m=m+1;
    end
end

for idcount=1:1%length(id_cl)
    tic
    ...
    for m=5:5:(length(B)-mod(length(B),5))
        
        k=1; %index for number of pts lying on this cross section
        tic
        P1=cross(N(m,1:3),X(1:3)); %vector lying on cross section
        ...
        for theta=2*pi/20:2*pi/20:2*pi
            ...
            if thetapts>2
                %k-thetapts to k-1 contains pts for this theta
                counterpts=1;
                for ptslineind=k-thetapts:k-1;
                   ptsline(counterpts,2:4)=Pint{m/5,ptslineind}(1,1:3);%pt lying on this line
                   ptdist=norm(B(m,1:3)-Pint{m/5,ptslineind}(1:3)); %distance to each pt on line
                   % ptsline(counterpts,2)=counterpts;
                   ptsline(counterpts,1)=ptdist;
                ...
                pqr=1;
                for pq=2:1
                    Pint{m/5,k-pq}(1,1:3)=ptsline(pqr,2:4);
                    pqr=pqr+1;
                end
                k=k-(thetapts-2); %move back k to where we want it to be, only two pts for each line
            end
        end
        toc
        m/5
        (length(B)-mod(length(B),5))/5
    end
    %% Plotting the mesh
    figure
    [l w]=size(Pint);
    ...
    tort_disp_maxdia=d(globmaxdia_ind*5);
