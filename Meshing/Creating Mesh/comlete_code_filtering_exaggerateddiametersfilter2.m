...
for idcount=3:3%length(id_cl)
    ...
    if change>=.2 | pt_counter~=40 | triflag~=0
        for i=1:w
            Pint{end,i}=[];
        end
        flag=1;
    ...
    for i=1:l-flag
        ...
        for j=1:w/2
            dist(j)=para{i,1}(j*2,1)+para{i,1}(j*2-1,1);
        end
        ...
        for j=1:w
            if isempty(Pintx{i,j})==0
                ...
                [azi2,ele2,r2]=cart2pol(b(1),b(2),b(3)); %spherical local coord of current pt
