...
for i=1:length(list)
    if isempty(strfind(list(i,:),'_T'))==0
        if isempty(strfind(list(i,:),'P07'))==0
            id_s(k)=i;
            k=k+1
        end
    end
    if isempty(strfind(list(i,:),'ct_P07'))==0
        id_cl(m)=i;
        m=m+1
    end
end

for idcount=1:length(id_s)
    ...
    filename = ['C:\Users\akshayrao\Documents\Rao Summer 17\AAA\Hamid_to_Akshay\P01to14_mindia2\',a];
    delimiter = ' ';
