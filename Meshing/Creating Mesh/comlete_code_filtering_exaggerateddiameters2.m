...
for idcount=2:2%length(id_cl)
    ...
    for m=4*34:4:4*39%(length(B)-mod(length(B),4)) %index for moving through points of Center line
        ...
        pt_counter
        change
        if pt_counter>36 & pt_counter<41 & change<.25%only include CS if the number of pts is 40 or less
            pint_ind=pint_ind+1;
        end
        toc
        m/4
