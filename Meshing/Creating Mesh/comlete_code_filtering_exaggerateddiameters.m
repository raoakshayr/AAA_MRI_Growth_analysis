...
for idcount=2:2%length(id_cl)
    ...
    for m=4*30:4:(length(B)-mod(length(B),4)) %index for moving through points of Center line
        ...
        if pint_ind>1 & pt_counter>30
            ...
            if counter>1
                maxchange=abs((pt_dist_max(pint_ind)-pt_dist_max(pint_ind-1))/pt_dist_max(pint_ind-1))
        ...
        pt_counter
        maxchange
        if pt_counter>36 & pt_counter<41 & maxchange<.25%only include CS if the number of pts is 40 or less
            pint_ind=pint_ind+1;
        end
        toc
        m/4
