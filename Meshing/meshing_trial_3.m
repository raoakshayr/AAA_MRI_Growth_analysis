    ...
    for theta=2*pi/10:2*pi/10:2*pi
        ...
        for i=1:length(T)
            ...
            if(k1*k2<0 | k2*k3<0 | k1*k3<0)
                ...
                if u>=0 & v>=0 &u+v<=1
                    tri(k)=T(i,1);%storing the index of the triangle which intersects and lies on either side of the cross section
                    k=k+1;
...
for i=1:length(tri)
    scatter3(T{tri{1,i},3}(1,1),T{tri{1,i},3}(1,2),T{tri{1,i},3}(1,3));
    hold on;
end