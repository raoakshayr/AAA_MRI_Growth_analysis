...
for pat=1:length(A)
    for j=1:length(A{pat,1})
        A{pat,8}(j,1)=A{pat,1}(j,1)*A{pat,10};
    end
...
for i=1:length(A)
    ...
    A{i,12}=A{i,8}(2,1)./A{i,1}(2,1);
    A{i,7}=A{i,7}+A{i,6}(1,1);
    %original time differences with time shift
end

figure;
for i=1:length(A)
plot(A{i,6}(:,1),A{i,3}(:,1),'g','linewidth',1.0);
%plot time and diameter with compression and stretching
text(A{i,6}(length(A{i,6}(:,1)),1),A{i,3}(length(A{i,6}(:,1)),1),A{i,5},... 
     'HorizontalAlignment','right',...
     'FontSize',9)
 
hold on; 
plot(A{i,7}(:,1),A{i,3}(:,1),'r','linewidth',1.0);
text(A{i,7}(length(A{i,7}(:,1)),1),A{i,3}(length(A{i,7}(:,1)),1),A{i,5},... 
     'HorizontalAlignment','right',...
     'FontSize',9)
%plot time and diameter without compression and stretching
