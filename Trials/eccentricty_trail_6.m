...
for i=1:length(cs_pts)
    ...
    for j=1:length(cs_pts{i,2})
        ...
        cs_pts{i,6}(j,1)=cos(theta);
        cs_pts{i,8}(j,1)=sin(theta);
        e=.1;
        if (theta<pi/2+e & theta>pi/2-e)
            theta
            cs_pts{i,7}(p,1:3)=cs_pts{i,2}(j,1:3);
            p=p+1;
        end
        
%         if ((abs(cos(theta)))<.1 & sin(theta)>0) %pts which are perpendicular to furthest pt vector and lie to one side
...
%         end
%         
%         if (cos(theta))<-.9 %pts which are on the other side of the major axis
...
for i=1:length(cs_pts{1,7})
    scatter3(cs_pts{1,7}(:,1),cs_pts{1,7}(:,2),cs_pts{1,7}(:,3));
    hold on;
end
